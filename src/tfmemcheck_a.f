c$Header: /SAD/cvsroot/oldsad/src/tfmemcheck.f,v 1.61.2.4 2012/07/31 05:16:17 amorita Exp $
      ...
      integer*8 kx,ktavaloc,ktadaloc,kaxi,kax5,kt,ka,kax
      integer*8 ip1,ip0,ip,ix,jx,i,j,k,i1, ki
      integer*4 isp1,irtc,ig,nc,l,
     $     nf,mf,isp0,m,itfmessage,ih
      real*8 xnmem,xnnet
      ...
      if(check)then
        ...
        do l=1,levele
          ...
          do while(i .ne. j)
            if(i .le. 0)then
              write(*,*)
     $       'Wrong temporary element:',i
              go to 9100
            endif
            ki=iand(ktfmask,klist(i))+i+2
            if(.not. tfcheckelement(ki,.true.))then
              call tfdebugprint(ki,'Error in temporary element',3)
              go to 9100
            endif
            i1=i
            i=ktfaddr(klist(i))
     ...
     $           klist(ip1+1),' index =',ix
          ...
          if(ix .lt. nindex)then
            m=ix+1
            if(m .gt. 6 .and. klist(ip+6) .ne. m)then
              write(*,*)
     ...
     $             ' index =',m,' stored size =',klist(ip+6)
      ...
      if(freel)then
        ...
        do i=1,nf*2,2
          kaxi=ktavaloc(0,2)
          rlist(kaxi+1)=ktastk(isp0+i)
          rlist(kaxi+2)=ktastk(isp0+i+1)
          klist(kax5+(i+1)/2)=ktflist+kaxi
        enddo
        isp=isp0
        klist(kax+5)=ktflist+kax5
      else
        kax=ktavaloc(-1,4)
      endif
      rlist(kax+1)=xnnet
      rlist(kax+2)=xnmem
      rlist(kax+3)=dble(nf)
      rlist(kax+4)=mf+xnnet-xnmem
      kx=ktflist+kax
      irtc=0
      return
 9000 name=tfgetstr(i+5,nc)
 ...
 9200 irtc=itfmessage(999,'General::memory',' ')
      ...
      include 'inc/TFSTK.inc'
      include 'inc/TFMEM.inc'
      integer*8 kx, k0,kad,kad0,kadi,kadi0,ks,ii, kt,ka, icont
      integer*8 i,j,k,i1, k1
      integer*4 irtc,ig,nc,l, m,itfmessage,ih
      character*16 tfgetstr,name
      ...
      do m=0,nsymhash
        ...
        do while(i .ne. j)
          ...
          if(ilist(1,i-1) .ne. nc/8+8)then
            write(*,*)'Inconsistent loc table size: ',ilist(1,i-1),
     $           ' <> ',nc/8+8
            go to 9000
          endif
          k=klist(i+1)
     ...
     $                 ' <>',kad0
              ...
              enddo
            enddo
c     if(ilist(1,k+2) .lt. 0)then
c     write(*,*)'Negative attribute: ',ilist(1,k+2)
c     go to 9000
c     endif
            ...
            if(ktfreallistq(ka))then
              do i=1,m
                if(ktfnonrealq(klist(ka+i)))then
                  call tfdebugprint(klist(ka+i),
     $             'Non-real in a real list:',3)
                  return
                endif
              enddo
            else
...
c          endif
     ...
     $           klist(ip+1),' index =',ix
            ...
            if(klist(ip) .lt. 128 .or. klist(ip) .gt. 2**30)then
              write(*,*)'tfck-1 ',klist(ip),ip,ip1,ix,jx
              v=rlist(7)
            endif
            ip1=ip
            ip=klist(ip1)
...
c        call tfdebugprint(it,ia,v,tag,3)
      endif
      irtc=-1
      return
      end
