...
c     
 ...
 12   if(word == ' ')then
      ...
      if(word == 'UNTIL')then
        if(levelr <= 0)then
          call termes(lfnm,'?UNTIL without REPEAT.',' ')
          go to 2
        endif
      ...
      elseif(word == 'BYE')then
        ...
        if(err)then
          if(byeall)then
            ...
            call termes(icslfnm(),'BYE without VISIT.',' ')
            go to 2
          endif
        endif
        call tclrpara
...
c$$$          endif
        ...
        call tffsfreefix(frefix,flv%nvar,lfnm)
...
c                   directio_vector(angle)
        wl=getva(exist)
        if(.not. exist) then
          call termes(icslfnm(),'? syntax:DELW <WAVE LENGTH> <amplitude>'//
...
c End of the lines added by N. Yamamoto Apr. 25, '93
     ...
     1    flv%kfit,flv%mfitp,flv%ifitp,flv%ifitp1,exist)
      ...
      elseif(word == 'SBUNCH')then
        call termes(icslfnm(),'?SBUNCH is obsolete.',' ')
        go to 10
c        iwakepold=ifwakep
c        call tfgetr(rlist(iwakepold+1),1.d0,word,lfno,exist)
c        go to 31
      elseif(word == 'DBUNCH')then
        call termes(icslfnm(),'?DBUNCH is obsolete.',' ')
        go to 10
c        iwakepold=ifwakep
...
c        go to 32
      elseif(word == 'SLICE')then
        call termes(icslfnm(),'?SLICE is obsolete.',' ')
        go to 10
c        iwakepold=ifwakep
c        ns=ilist(1,iwakepold+2)
c        call tfgeti(ns,1.d0,word,lfno,exist)
c        if(ns <= 0)then
c          call termes(icslfnm(),'?Parameter out of range in SLICE.',' ')
c          go to 2
c        endif
...
31    if(.not. exist)then
 ...
 1000 continue
      if(busy)then
        call termes(icslfnm(),'?Recursive CAL or GO',' ')
        go to 2
      endif
      busy=.true.
      call tfsetparam
     ...
     $         flv%kfit(k1) <= mfitchi3)then
          ...
          if(nfcol > maxcond)then
            call termes(icslfnm(),'?Too many fit conditions.',' ')
            go to 8810
          endif
          flv%kfitp(nfcol)=k1
        endif
...
c        enddo
        jfam(0)=0
      elseif(err)then
        call termes(icslfnm(),'Error in InitialOrbits',' ')
        go to 8810
      else
        inicond=.false.
        nfr=-1
        ...
        if(nfr > 0 .and. dpmax == 0.d0)then
          call termes(icslfnm(),'?No momentum bandwidth.',' ')
          go to 8810
        endif
        if(nfr .lt. 0)then
          nfr=0
...
c        write(*,*)'tffsa-setupwake-done ',nwakep
        if(irtc /= 0)then
          call termes(icslfnm(),'?Error in WakeFunction.',' ')
          go to 8810
        endif
        if(nwakep == 0)then
          wake=.false.
...
c          call tmov(rlist(iffssave+2),ffv,nxh)
     ...
     $       nvevx(i)%ivvar /= nelvx(nvevx(i)%ivarele)%ival)then
      ...
      if(tflistq(kx,klx))then
        ...
        nfr=(n+1)/2
        if(nfr > ndimmax)then
          call termes(icslfnm(),'Too many initial conditions.',' ')
          return
        endif
        nfr1=-(n/2)
        iuid(-nfr)=-1
