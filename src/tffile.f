      ...
      elseif(abbrev(word,'EXE_CUTE','_'))then
        ...
        if(.not. ktfstringq(kx,str))then
          call termes('?Missing string for EXE_CUTE.',' ')
          init=.true.
          return
        endif
        ipoint=next
     ...
     1           ERR=6101)
          ...
          call termes('?Missing filename for OUT_PUT',' ')
          init=.true.
          return
        endif
        lfno=lfno1
...
c        call tfdebugprint(kx,'IN',1)
     ...
     $           str%str(1:str%nch))
          ...
          call termes('?Missing filename for IN_PUT',' ')
          init=.true.
          return
        endif
        ipoint=next
        if(lfnp .ge. maxlfn)then
          call termes('?Number of input files exceeds limit',' ')
          init=.true.
          return
        endif
        if(lfni1 .le. 0)then
