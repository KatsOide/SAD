c     Don't confuse, Emacs. This is -*- fortran -*- mode!
     ...
     $     modethrow,iordless
      ...
      integer*4 idtype(HTMAX),idval(HTMAX)
      integer*4 ilistroot
c      integer*8, target :: klist(RBASE:RBASE+MAXMEM0-1)
      ...
      module tfmem
      implicit none
      integer*8, parameter :: mpsize=2**20
     ...
     $     kcpthre  = 2**35, nlarge = 2**30, nitaloc0 = 2**31
      integer*4 icbk
...
c        kcpklist0=transfer(c_loc(kdummy),int8(0))-kcpoffset-8
        ...
        call c_f_pointer(c_loc(klist(1)),vstk,[klistlen])
        call c_f_pointer(c_loc(klist(1)),rtastk,[klistlen])
        call c_f_pointer(c_loc(klist(1)),ktastk,[klistlen])
        call c_f_pointer(c_loc(klist(1)),dtastk,[klistlen])
        call c_f_pointer(c_loc(klist(1)),ilist,[2,klistlen])
        call c_f_pointer(c_loc(klist(1)),itastk,[2,klistlen])
        call c_f_pointer(c_loc(klist(1)),ivstk,[2,klistlen])
        call c_f_pointer(c_loc(klist(1)),jlist,[8,klistlen])
        return
        end subroutine

        subroutine tfstk2init
        use iso_c_binding
        implicit none        
        call c_f_pointer(c_loc(klist(ivstkoffset+1)),vstk2,[klistlen])
        call c_f_pointer(c_loc(klist(ivstkoffset+1)),rtastk2,[klistlen])
        call c_f_pointer(c_loc(klist(ivstkoffset+1)),ktastk2,[klistlen])
        call c_f_pointer(c_loc(klist(ivstkoffset+1)),dtastk2,[klistlen])
        call c_f_pointer(c_loc(klist(ivstkoffset+1)),
     $       itastk2,[2,klistlen])
        call c_f_pointer(c_loc(klist(ivstkoffset+1)),
     $       ivstk2,[2,klistlen])
        return
...
c     write(*,*)'with ',ilist(1,ka-1),ktfaddr(klist(ka-2))
 ...
 10     if(present(kl))then
     ...
     $       k_descr(ktfoper+mtfleftbrace)))
        ...
        implicit none
        integer*8 ka
        tfonstackq=ka .ge. isporg .and. ka .le. isporg+ivstkoffset
        return
        end function
      ...
      module ffslocal
        integer*4 maxcond,lblname
        parameter (maxcond=4001)
        parameter (lblname=MAXCOND/4)

        type ffslocalv
          sequence
          integer*8 iut
          real*8 ctime0,ctime2,rsconv,fitval(maxcond)
     ...
     $         blname(lblname),pading,mcommon
      ...
      implicit none
      integer*4
     $     mfitax,mfitbx,mfitnx,mfitay,mfitby,mfitny,
     $     mfitex,mfitepx,mfitey,mfitepy,
     $     mfitr1,mfitr2,mfitr3,mfitr4,
     $     mfitdx,mfitdpx,mfitdy,mfitdpy,mfitdz,mfitddp,mfitdetr,
     $     mfitpex,mfitpepx,mfitpey,mfitpepy,
     $     mfittrx,mfittry,mfitleng,
     $     mfitgx,mfitgy,mfitgz,mfitchi1,mfitchi2,mfitchi3
      parameter (
     ...
     $     mfitchi1=mfitgz+1,mfitchi2=mfitchi1+1,mfitchi3=mfitchi2+1)
      integer*4 ntwissfun,mfito,mfit,mfit1
      parameter (ntwissfun=mfitddp,mfito=mfittry,mfit=mfitchi3,
     $     mfit1=mfit+12)
      end module
     ...
     $       ifmast,iftouchele,iftouchv,lfnp,iffserr,ifwakep,ifivcomp
        real*8 emx,emy,dpmax,geo0(3,4),xixf,xiyf
        integer*4 ndim,ndima,nele,nfit,marki,iorgx,iorgy,iorgr,
     $       mfpnt,mfpnt1,id1,id2,nve,ielmhash
      end type

      type (ffsv), target, save:: ffv

      integer*8, pointer :: ifaux,ifibzl,ifmult,ifklp,ifival,iftwissp,
     $     iftwis,ifpos,ifgeo,ifsize,ifgamm ,ifele,ifcoup,
     $     iferrk,ifvarele,ifvvar,ifvalvar,ifele1,ifele2,
     $     ifmast,iftouchele,iftouchv,lfnp,iffserr,ifwakep,ifivcomp
      real*8, pointer :: emx,emy,dpmax,xixf,xiyf
      real*8, pointer, dimension(:,:) :: geo0
      integer*4, pointer :: ndim,ndima,nele,nfit,marki,iorgx,iorgy,
     $     iorgr,mfpnt,mfpnt1,id1,id2,nve,ielmhash
