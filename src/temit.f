...
c     call tsymp(trans)
 ...
 3101 call tmov(trans,r,36)
     ...
     $       dtfcopy1(kxm2l(codin,0,6,1,.false.))
      ...
      if(synchm)then
        if(hvc0 .ne. 0.d0)then
c          alphap=imag(cd(6))**2/pi2/hvc0*pgev/cos(phirf)*p0/h0
          alphap=-imag(cd(6))*abs(imag(cd(6)))
     $          /pi2/abs(hvc0)/vcalpha*pgev/cos(phirf)*p0/h0
        else
          alphap=0.d0
        endif
        ...
        omegaz=sqrt(abs(alphap*pi2*hvc0/pgev*cos(phirf)))*omega0/pi2
      endif
      if(vceff .ne. 0.d0)then
