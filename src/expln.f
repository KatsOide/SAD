      ...
      integer*4 lpname,mctaloc,italoc,idi
      real*8 frand
...
c      newadr=mfalloc(blksz)
 ...
 1100 continue
      i0=i0+1
      if (i0 .gt. ilist(1,idx)) go to 1400
     ...
     &           'definition contains a loop.',0,16)
         ...
         if (pstack .gt. STACSZ) then
            call errmsg('expnln',
     &           'stack overflow: Recursive definition?'
     &           ,0,16)
         endif
         stack(1,pstack)=idx
         stack(2,pstack)=i0
         stack(3,pstack)=direct
         ...
         do k=1,abs(ilist(1,idx+i))-1
            ...
            if (pstack .gt. STACSZ) then
               call errmsg('expnln',
     &              'stack over flowed. Definition of line is too long'
     &              ,0,16)
            endif
            stack(1,pstack)=idval(idxi2)
            stack(2,pstack)=0
            stack(3,pstack)=direct
         end do
         idx=idval(idxi2)
     ...
     &        'warnig:unable to expand line',0,0)
         ...
         do j=1,abs(ilist(1,idx+i))
            ...
            if(llen .ge. blksz) then
               newadr=italoc(2*blksz-1)-1
c    The original was below.  7/25/2011 K. Oide
c               newadr=mfalloc(2*blksz)
               if(newadr .le. 0) then
                  call errmsg('expln',' cannot allocate memory',0,32)
                  stop
               end if
               ilist(2,newadr)=0
               do k=1,llen-1
...
c     
 1400 continue
      ...
      if( llen+1  .gt. blksz ) call errmsg('doexpn','expanded line is'
     &     //' too long.',0,32)
      call freeme(ilist(2,idx0)+llen+1,blksz-llen-1)
c     
