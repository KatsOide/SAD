      ...
      integer*8 ktatwissaloc,kax,kaxi
      integer*4 isp1,irtc,narg,i,itoff,m,nc,j,
     $     isp0,nd,kt,itfmessage
      real*8 ftwiss(27),pe(4),tfgettwiss,tphysdisp
      logical*4 over,ref
     ...
     $               +itastk(2,isp)-1
              ...
              call qtwissfrac(rlist(kax+1),
     $             ilist(1,ilattp+1),rlist(iftwis),
     $             rlist(ifgamm),itastk(2,isp),
     $             vstk2(isp),over)
            endif
          else
            m=isp-isp0
     ...
     $                 +itastk(2,isp0+i)-1
                ...
                call qtwissfrac(rlist(kaxi+1),
     $               ilist(1,ilattp+1),rlist(iftwis),
     $               rlist(ifgamm),itastk(2,isp0+i),
     $               vstk2(isp0+i),over)
              endif
            enddo
          endif
 ...
 110    itoff=(2*ndim+1)*nlat*(kt-1)+ndim*nlat+int(iftwis)
        if(narg .eq. 1)then
          ...
          elseif(kt .ge. mfitpex .and. kt .le. mfitpepy)then
            do i=1,nlat
              call tgetphysdisp(rlist(iftwis),i,pe)
        ...
        elseif(narg .eq. 2)then
          ...
          if(isp .eq. isp0+1)then
            if(vstk2(isp) .eq. 0.d0)then
              ...
              elseif(kt .ge. mfitpex .and. kt .le. mfitpepy)then
                call tgetphysdisp(rlist(iftwis),itastk(2,isp),pe)
...
c     $             itastk(2,isp),vstk2(isp)
     ...
     $                 vstk2(isp0+i),over)
            ...
            elseif(kt .ge. mfitpex .and. kt. le. mfitpepy)then
              do i=1,m
                if(vstk2(isp0+i) .eq. 0.d0)then
                  call tgetphysdisp(rlist(iftwis),itastk(2,isp0+i),pe)
                  rlist(kax+i)=pe(kt-mfitpex+1)
                else
                  call qtwissfrac(ftwiss,
     $                 ilist(1,ilattp+1),rlist(iftwis),
     ...
     $           '"name of component for #2"')
      ...
      integer*8 kax
      integer*4 mode
      kax=ktavaloc(mode,27)
      rlist(kax+ntwissfun+1:27)=0.d0
      rlist(kax+22)=1.d0
      ktatwissaloc=kax
      return
      end
 ...
 9010 irtc=6
      ...
      implicit none
      type (sad_descriptor) kx
      integer*4 irtc,id,lenw,it,ia,iv,isps,l,iax
      real*8 tfkeyv
      character*(*) keyword
      ...
      elseif(keyword .eq. 'DEFAULT')then
        ...
        if(iv .eq. 0)then
          ...
          key=tfkwrd(idtype(ilist(1,ilattp+ia)),iv)
        endif
        Kx=kxsalocb(-1,key,lenw(key))
      elseif(keyword .eq. 'KEYWORDS')then
        l=0
        id=idtype(ilist(1,ilattp+ia))
        isps=isp
        key='-'
        do while(key .ne. ' ')
          l=l+1
      ...
      elseif(keyword .eq. 'TYPE')then
        kx=dfromr(dble(idtype(ilist(1,ilattp+ia))))
      elseif(keyword .eq. 'TYPENAME')then
        key=pname(kytbl(0,idtype(ilist(1,ilattp+ia))))
     ...
     $       ifany1(name(1:nc),nc,'*%{<|',1) .gt. 0)then
          do i=1,nele
            if(tmatch(pname(ilist(1,ilattp+ilist(i,ifklp))),
     $           name(1:nc)))then
              isp=isp+1
              itastk(1,isp)=i
      ...
      elseif(keyword .eq. 'EXPAND')then
        ...
        do i=1,nlat-1
          ...
          if(ie .gt. 0)then
            ...
            if(iv .gt. 0)then
              k=ilist(ie,ifklp)
              rlist(ilist(2,ilattp+i)+iv)=
     $             rlist(ifcoup+i-1)*
     $             rlist(iferrk+i*2-2)/rlist(iferrk+k*2-2)*
     $             rlist(ilist(2,ilattp+k)+iv)
            endif
          endif
        enddo
        kx%k=ktfoper+mtfnull
      ...
      implicit none
      type (sad_descriptor) kx
      integer*8 kax,ktfgeol,kai,j,i
      integer*4 irtc,lp,lenw,lxp,nv,lt,itoff,kk,ia,ip,isp1
     ...
     $       rlist(ifgamm+min(nlat-1,ia))*v)**2))
      elseif(keyword(1:3) .eq. 'SIG' .or. keyword(1:4) .eq. 'SIZE')then
        if(keyword(1:3) .eq. 'SIG')then
          call tfbeamfrac(rlist(iftwis),rlist(ifgamm),ia,v,0.d0,beam)
          call tfbeamkey(keyword(4:),i,j,irtc)
        else
          call tfsizefrac(rlist(ifsize),ia,v,beam)
      ...
      elseif(keyword .eq. 'TYPENAME')then
        if(ia .eq. nlat)then
          ...
          name=pname(kytbl(0,idtype(ilist(1,ilattp+ia))))
          kx=kxsalocb(-1,name(2:),lenw(name)-1)
        endif
      elseif(keyword .eq. 'NAME')then
        call elname(ilist(1,ilattp+1),ia,ilist(1,ifmult),name)
        kx=kxsalocb(-1,name,lenw(name))
      elseif(keyword .eq. 'ELEMENT')then
        if(ia .eq. nlat)then
          name='$$$'
        else
          name=pname(ilist(1,ilattp+ia))
        endif
        kx=kxsalocb(-1,name,lenw(name))
      elseif(keyword .eq. 'POSITION')then
        kx=dfromr(dble(ia))
      elseif(keyword .eq. 'GEO')then
        ...
        if(fr .eq. 0.d0)then
          ...
          lt=idtype(ilist(1,ilattp+lxp))
          nv=kytbl(kwmax,lt)-1
          call tmov(rlist(ilist(2,ilattp+lxp)+1),vsave,nv)
          call qfraccomp(ilist(1,ilattp+lxp),0.d0,fr,.false.,chg)
          if(chg)then
            geo1=rlist(j+1:j+12)
            pos0=rlist(ifpos+lxp)
            gam0=rlist(ifgamm+lxp)
            call tfgeo1(ilist(1,ilattp+1),
     ...
     $           lxp,lxp+1,.true.,.false.)
          ...
          call tmov(vsave,rlist(ilist(2,ilattp+lxp)+1),nv)
        endif
        kx%k=ktflist+kax
     ...
     $           lxp,lxp+1,.true.,.false.)
          ...
          call tmov(vsave,rlist(ilist(2,ilattp+lxp)+1),nv)
          call qtwissfrac(vtwiss,
     $         ilist(1,ilattp+1),rlist(iftwis),
     $         rlist(ifgamm),lxp,fr,over)
     ...
     $         rlist(ifgamm),lxp,fr,over)
      ...
      elseif(keyword .eq. 'DK')then
        ...
        if(ia .lt. nlat)then
          kx=dfromr(tfkeyv(ia,keyword,ip,.false.))
          if(.not. ref)then
            kx%k=ktfref+ip
          endif
          tparaed=.false.
 ...
 9000 irtc=itfmessage(9,'General::wrongval',
     $     '"coordinae","X, PX, Y, PY, Z, DP"')
      ...
      type (sad_descriptor) k
      integer*8 kav,ka
      integer*4 narg,isp0,irtc,i,nc,ifany1,ielmf,itfmessage,
     $     j,itehash,l
      real*8 r
     ...
     $       ifany1(name(1:nc),nc-2,'*%{|',1) .eq. 0)then
          ...
          if(j .ne. 0)then
            do i=j,j+ilist(1,ielmhash+ka)-1
              l=ilist(1,i)
              if(name2(1:nc-2) .eq. pname(ilist(1,ilattp+l)))then
                isp=isp+1
                itastk(1,isp)=ilist(l,ifele1)
                itastk(2,isp)=l
                vstk2(isp)=0.d0
     ...
     $       ifany1(name(1:nc),nc,'*%{|',1) .gt. 0)then
          do i=1,nlat
            if(temat(ilist(1,ilattp+1),i,ilist(1,ifmult),
     $           name1,name(1:nc)))then
              isp=isp+1
              itastk(1,isp)=ilist(i,ifele1)
              itastk(2,isp)=i
              vstk2(isp)=0.d0
...
c     end   initialize for preventing compiler warning
      ...
      return
 1    if(i .gt. 0)then
        ia=ilist(2,ilattp+i)+l
        tfkeyv=rlist(ia)
      elseif(saved)then
        ia=idval(ilist(1,ilattp+kl))+l
        tfkeyv=rlist(ia)
      else
        if(l .eq. ilist(-i,ifival))then
          ia=ilist(2,ilattp+kl)+l
     ...
     $         '"name of component"')
        ...
        itfloc=ielm(ilist(1,ilattp+1),name(1:nc),
     $       1,ilist(1,ifmult),exist)
        if(.not. exist)then
