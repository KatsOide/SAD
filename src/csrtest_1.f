 ...
 9010 irtc=itfmessage(9,'General::wrongtype',
     ...
     $     fudge0,fudge1,fudge2,fudge3
      ...
      nm=0
      do i=1,m1
        nm=nm+rlist(ipipe+i-1)
        an=max(an,rlist(ipipe+i-1))
      enddo
      n1=an
      if(mode .eq. 1000)then
        call csrhelmat(iax,rlist(ipipe),dx,dy,m1,n1,ic,nm)
      else        
        iaind=italoc(((n1+2)*(m1+2)+1)/2+1)
        call csrind(ilist(1,iaind),rlist(ipipe),m1,n1,mm)
        if(mode .lt. 100)then
...
c 32: Er -> Ey
 ...
 9030 irtc=itfmessage(9,'General::narg',
     $       '"1"')
      ...
      subroutine csrhelmat(iax,pipe,dx,dy,mx,my,ic,nm)
      implicit none
      integer*4 mx,my,nphi,ic,nm,nphimax,i,j,ind,iax,itfm2l,
     $     nphi1
      parameter (nphimax=4000)
      real*8 pipe(mx),phix(0:nphimax),phiy(0:nphimax),
     $     tx(nphimax),ty(nphimax),xc(nm),yc(nm),
     $     psi(nm*2,nm*2),dx,dy,sx,sy,r,x,y,cs
      real*8 pi
      ...
      cs=dx*dy/4.d0/pi
      nphi1=nphi-1
      call csr2dpoisson1(phix,phiy,tx,ty,
     $     psi,xc,yc,cs,nphi1,nm)
      iax=itfm2l(psi,nm*2,nm*2,nm*2,.false.)
      return
      end

      subroutine csr2dpoisson1(phix,phiy,tx,ty,
     $     psi,xc,yc,cs,nphi,nm)
      implicit none
      integer*4 nphi,i,j,k,nphi1,nphi2,nphi3,nv,nm
      real*8 phix(0:nphi),phiy(0:nphi),tx(2*nphi-1),ty(2*nphi-1),
     $     psi(nm*2,nm*2),xc(nm),yc(nm),
     $     a(2*nphi-2,4*nphi-4),er(4*nphi-4,2,nm),b(2*nphi-2,2,nm),
     $     xp,yp,xf,yf,exa1(nm),eya1(nm),exa2(nm),eya2(nm),
     $     rsqi1,rsqi2,rsqi1a,rsqi2a,cs,eps,w0,w00,sa,ws,ws1,
     $     w1,w2,wxx,wxy,wyx,wyy,d,t1,t2,txp,typ
c      logical*4 isnan
      real*8 pi
      parameter (pi=3.14159265358979324d0)
      sa=cs*4.d0*pi
      ...
      do i=1,nphi1
        ...
        do k=1,nm
          ...
          w1=log(rsqi1)+ws1*rsqi1
          t1=-2.d0*atan2(yf-yp,xf-xp)
          w2=log(rsqi2)+ws1*rsqi2
          t2=-2.d0*atan2(yf+yp,xf-xp)
          rsqi1a=rsqi1*(1.d0+2.d0*ws1*rsqi1)
          rsqi2a=rsqi2*(1.d0+2.d0*ws1*rsqi2)
          wxx= (xf-xp)*(rsqi1a+rsqi2a)*d
          wxy=((yf-yp)*rsqi1a+(yf+yp)*rsqi2a)*d
          wyx= (xf-xp)*(rsqi1a-rsqi2a)*d
          wyy=((yf-yp)*rsqi1a-(yf+yp)*rsqi2a)*d
          b(i      ,1,k)=-(w1+w2)*txp-(t1+t2)*typ
          b(nphi1+i,1,k)=2.d0*wxy*txp*typ+wxx*(txp**2-typ**2)
          b(i,      2,k)=-(w1-w2)*typ+(t1-t2)*txp
          b(nphi1+i,2,k)=2.d0*wyx*txp*typ-wyy*(txp**2-typ**2)
        enddo
        do j=1,nphi1
          xp=phix(j)
          yp=phiy(j)
          ...
          wyx= (xf-xp)*(rsqi1a-rsqi2a)*d
          wyy=((yf-yp)*rsqi1a-(yf+yp)*rsqi2a)*d
          a(i,            j)=-(w1+w2)*txp-(t1+t2)*typ
          a(i,      nphi1+j)=-(w1-w2)*typ+(t1-t2)*txp
          a(nphi1+i,      j)=
     $         -2.d0*wxy*txp*typ-wxx*(txp**2-typ**2)
          a(nphi1+i,nphi1+j)=
     $         -2.d0*wyx*txp*typ+wyy*(txp**2-typ**2)
        enddo
        do j=1,nphi
          ...
          if(j .ne. 1 .and. j .ne. nphi)then
            a(i,      nphi2+j)=-(w1+w2)*txp-(t1+t2)*typ
            a(i,      nphi3+j)=-(w1-w2)*typ+(t1-t2)*txp
            a(nphi1+i,nphi2+j)=
     $           -2.d0*wxy*txp*typ-wxx*(txp**2-typ**2)
            a(nphi1+i,nphi3+j)=
     $           -2.d0*wyx*txp*typ+wyy*(txp**2-typ**2)
     ...
     $           -2.d0*wxy*txp*typ-wxx*(txp**2-typ**2)
        ...
        enddo
      enddo
      call tsolvm(a,b,er,nphi2,nv,2*nm,
     $     nphi2,nphi2,nv,eps,.true.)
      do k=1,nm
        xf=xc(k)
        yf=yc(k)
        do i=1,nm
          if(i .ne. k)then
            rsqi1=1.d0/((xf-xc(i))**2+(yf-yc(i))**2)
            w1=log(rsqi1)+ws*rsqi1
          else
            w1=w0
          endif
          t1=-2.d0*atan2(yf-yc(k),xf-xc(k))
          rsqi2=1.d0/((xf-xc(i))**2+(yf+yc(i))**2)
          w2=log(rsqi2)+ws*rsqi2
          t2=-2.d0*atan2(yf+yc(k),xf-xc(k))
          exa1(i)= w1+w2
          eya1(i)= t1+t2
          exa2(i)=-t1+t2
          eya2(i)= w1-w2
        ...
        do j=1,nphi1
          ...
          do i=1,nm
            exa1(i)=exa1(i)+
     $           (w1+w2)*er(j      ,1,i)-(t1-t2)*er(nphi1+j,1,i)
            eya1(i)=eya1(i)+
     $           (w1-w2)*er(nphi1+j,1,i)+(t1+t2)*er(j     ,1,i)
            exa2(i)=exa2(i)+
     $           (w1+w2)*er(j,      2,i)-(t1-t2)*er(nphi1+j,2,i)
            eya2(i)=eya2(i)+
     $           (w1-w2)*er(nphi1+j,2,i)+(t1+t2)*er(j     ,2,i)
          enddo
        enddo
        do j=1,nphi
          xp=(phix(j)+phix(j-1))*.5d0
          ...
          if(j .ne. 1 .and. j .ne. nphi)then
            do i=1,nm
              exa1(i)=exa1(i)+
     $             (w1+w2)*er(nphi2+j,1,i)-(t1-t2)*er(nphi3+j,1,i)
              eya1(i)=eya1(i)+
     $             (w1-w2)*er(nphi3+j,1,i)+(t1+t2)*er(nphi2+j,1,i)
              exa2(i)=exa2(i)+
     $             (w1+w2)*er(nphi2+j,2,i)-(t1-t2)*er(nphi3+j,2,i)
              eya2(i)=eya2(i)+
     $             (w1-w2)*er(nphi3+j,2,i)+(t1+t2)*er(nphi2+j,2,i)
            enddo
          else
            do i=1,nm
              exa1(i)=exa1(i)+
     $             (w1+w2)*er(nphi2+j,1,i)
              eya1(i)=eya1(i)+
     $             (t1+t2)*er(nphi2+j,1,i)
              exa2(i)=exa2(i)+
     $             (w1+w2)*er(nphi2+j,2,i)
              eya2(i)=eya2(i)+
     $             (t1+t2)*er(nphi2+j,2,i)
            enddo
          endif
        enddo
        do i=1,nm
