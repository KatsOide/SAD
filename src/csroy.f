     ...
     $     kax
      ...
      dt=min(1.d0,0.03d0/sqrt(abs(r56*r65)))
      call csroysetup(
     $     rlist(karho+1),
     $     lazl%rbody(1),
     $     rlist(kawi+1),
...
c
c   rho:     charge density in z, as the output tfcsrhaissin. Sum(rho) = 1.
...
c
      ...
      sigz=lal%rbody(4)
      sige=lal%rbody(5)
      bw=bw/nr/dz*4.d0/pi/(2.d0*pi*anus*sige*sigz)
      mm=nj*nphi
      call csroymat(kax,
     $     lawi   %rbody(1),
     $     laomega%rbody(1),
...
c
     ...
     $     zc,dk,zspan,pi,ox,ak,dt,csrwkick,
     $     r56,r65,rhoth,z,dpjn(mphi,nj),rhoj(nj)
      ...
      zc=nrh/2*dz
      rhoth=exp(-zspan**2/2.d0)/sqrt(2.d0*pi);
      do i=1,nrh
