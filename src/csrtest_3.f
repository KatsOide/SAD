...
c 32: Er -> Ey
 ...
 9030 irtc=itfmessage(9,'General::narg',
     ...
     $     psi(nm*2,nm*2),dx,dy,sx,sy,r,x,y,cs
      ...
      cs=dx*dy/4.d0/pi
      nphi1=nphi-1
      call csr2dpoisson1(phix,phiy,psi,xc,yc,cs,nphi1,nm)
      iax=itfm2l(psi,nm*2,nm*2,nm*2,.false.)
      return
      end

      real*8 function phis(x,y)
      real*8 x,y,atanz
      if(x .eq. 0.d0)then
        phis=0.d0
      else
        phis=-2.d0*y*atanz(x,y)+x*(log(x**2+y**2)-2.d0)
      endif
      return
      end
      ...
      if(y .eq. 0.d0)then
        ...
        atanz=(atan2(y,x)+atan2(-y,-x))*.5d0
      endif
      return
      end

      subroutine csr2dpoisson1(phix,phiy,psi,xc,yc,cs,nphi,nm)
      implicit none
      integer*4 nphi,i,j,k,nphi1,nv,nm
      real*8 phix(0:nphi),phiy(0:nphi),psi(nm*2,nm*2),xc(nm),yc(nm),
     $     a1(nphi-1,2*nphi-1),er1(2*nphi-1,nm),b1(nphi-1,nm),
     $     a2(nphi-1,2*nphi-1),er2(2*nphi-1,nm),b2(nphi-1,nm),
     $     xp,yp,xf,yf,exa1(nm),eya1(nm),exa2(nm),eya2(nm),
     $     rsqi1,rsqi2,rsqi1a,rsqi2a,cs,eps,w0,w00,sa,ws,ws1,w1,w2,
     $     phis,atanz
      real*8 pi,el
      ...
      do i=1,nphi1
        ...
        do k=1,nm
          xp=xc(k)
          yp=yc(k)
          b1(i,k)=-phis(xf-xp,yf-yp)-phis(xf-xp,yf+yp)
          b2(i,k)=-phis(yf-yp,xf-xp)+phis(yf+yp,xf-xp)
        enddo
        do j=1,nphi1
          xp=phix(j)
          yp=phiy(j)
        ...
        do j=1,nphi
          ...
          if(j .ne. 1 .and. j .ne. nphi)then
            ...
            a1(i,nphi1+j)=(w1+w2)*0.5d0
            a2(i,nphi1+j)=0.d0            
          endif
        enddo
      enddo
      call tsolvm(a1,b1,er1,nphi1,nv,nm,nphi1,nphi1,nv,eps,.true.)
      call tsolvm(a2,b2,er2,nphi1,nv,nm,nphi1,nphi1,nv,eps,.true.)
      do k=1,nm
        xf=xc(k)
        yf=yc(k)
        do i=1,nm
          ...
          exa1(i)= (w1+w2)*0.5d0
          eya1(i)= atanz(xf-xp,yf-yp)+atanz(xf-xp,yf+yp)
          exa2(i)= atanz(yf-yp,xf-xp)-atanz(yf+yp,xf-xp)
          eya2(i)= (w1-w2)*0.5d0
        enddo
        do j=1,nphi1
          xp=phix(j)
          ...
          rsqi2=1.d0/((xf-xp)**2+(yf+yp)**2)
          do i=1,nm
            exa1(i)=exa1(i)+(xf-xp)*(rsqi1+rsqi2)*er1(j,i)
            eya1(i)=eya1(i)+((yf-yp)*rsqi1+(yf+yp)*rsqi2)*er1(j,i)
            exa2(i)=exa2(i)+(xf-xp)*(rsqi1-rsqi2)*er2(j,i)
            eya2(i)=eya2(i)+((yf-yp)*rsqi1-(yf+yp)*rsqi2)*er2(j,i)
          enddo
        enddo
        do j=1,nphi
          xp=(phix(j)+phix(j-1))*.5d0
          ...
          if(j .ne. 1 .and. j .ne. nphi)then
            do i=1,nm
              exa1(i)=exa1(i)+
     $             (xf-xp)*(rsqi1+rsqi2)*er1(nphi1+j,i)
              eya1(i)=eya1(i)+
     $             ((yf-yp)*rsqi1+(yf+yp)*rsqi2)*er1(nphi1+j,i)
              exa2(i)=exa2(i)+
     $             (xf-xp)*(rsqi1-rsqi2)*er2(nphi1+j,i)
              eya2(i)=eya2(i)+
     $             ((yf-yp)*rsqi1-(yf+yp)*rsqi2)*er2(nphi1+j,i)
            enddo
          else
            do i=1,nm
              exa1(i)=exa1(i)+
     $             (xf-xp)*(rsqi1+rsqi2)*er1(nphi1+j,i)*.5d0
              eya1(i)=eya1(i)+
     $             ((yf-yp)*rsqi1+(yf+yp)*rsqi2)*er1(nphi1+j,i)*.5d0
            enddo
          endif
        enddo
        do i=1,nm
