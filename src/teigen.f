...
c     write(*,*)
      ...
      subroutine tqr3(a,w,eig)
      use tfstk, only: ktfenanq
      implicit none
      integer*4 , parameter ::itmax=100
      integer*4 ,parameter :: n=3,ndim=3,is=1,is1=2,is2=3,ib=1
      real*8 ,parameter ::vmax=1.d30,vmin=1.d0/vmax,
     $     decth=1.d-16,threj=1.d-8,alpha=1.d-7,
     $     amth=1.d-30
      integer*4 ie,i,j,iter,itm,i1,i2,i3,jm,j1
      real*8 , intent(inout)::a(n,n)
      real*8 , intent(out):: w(ndim,n),eig(2,n)
      real*8 aa(3),aa1(3)
      real*8 px(2),py(2)
      real*8 anorm,c,s,p,q,v1,v2,am,am1,w1,w2,w3,
     $     dec,sqrd,r,da,pk,pl,u,v,pm,pn,a1,a2,x,y,sa,
     $     r1,r2,s1
      logical*4 paired,jordan
      complex*16 ca,cc,cu,cr,ck,cm
      equivalence (ck,px),(cm,py)
...
c     write(*,'(1P4G15.7)')((vx(i)/vx(j)*a(i,j),j=1,4),i=1,4)
          ...
          enddo
          a1=a(ie,ie)-a(is,is)
          a2=a(is1,is1)-a(is,is)
          w1=((a1*a2-a(is1,ie)*a(ie,is1))/a(is1,is)+a(is,is1))
     1         *vx(is)/vx(is1)
          if(w1 == 0.d0)then
            w1=-alpha*a(is1,ie)*a(ie,is1)/a(is1,is)*vx(is)/vx(is1)
          endif
          w2=a(is1,is1)-a(ie,ie)-a2
c          w2=a(is1,is1)-a(is,is)-a1-a2
          w3=a(is2,is1)*vx(is2)/vx(is1)
          exit
        enddo do2
        a(is2,is)=0.d0
c        a(min(is2+1,ie),is)=0.d0
        am =hypot(w1,w2)
        am1=norm2([w1,w2,w3])
c        if(is > ib)then
c          if(am1 /= 0.d0)then
c            a(is,is-1)=a(is,is-1)*w1/am1*vx(is)
c          endif
c        endif
        do i=is-1,ie-2
          i1=i+1
