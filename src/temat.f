...
c      write(*,*)'temat ',i,latt(i),ilist(2,latt(i)),name(1:l)
      if(ifany1(word,lw,'*{|%',1) == 0)then
        if(name(1:l) == word(1:lw))then
c     Exact match with standard element name(elname)
          temat=.true.
        elseif(i == nlat)then
c     No more test for nlat($$$) 
          temat=.false.
        elseif(mult(i) == 0)then
c     Test implicit match with singlet element
          call elnameK(i,name1)
          temat=name1(1:lenw(name1)) == word(1:lw)
        elseif(nelvx(ilist(i,ifele1))%klp == i)then
c     Test implicit match with head of multiple elements
          temat=pname(idelc(i))(1:lpnamec(i)) == word(1:lw)
        else
          temat=.false.
        endif
      else
