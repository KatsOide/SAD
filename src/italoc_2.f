...
c      call tfreecheck('ktaloc-0 ',1,0,dble(n),irtc)
      if(n1 .lt. nindex)then
        ...
        if(ia .ne. i)then
          ...
          if(ic .ne. ia+1)then
            write(*,*)'ktaloc-1 ',ia,ic,klist(ic+6)
            call tfree(int(ic))
          endif
          ilist(1,ia-1)=m
          nnet=nnet+m
...
c      call tfreecheck('tsih-0 ',1,0,dble(ip),irtc)
      ...
      klist(i+3)=ip
      klist(ic+6)=m
      call tfreecheck('tsih-1 ',1,0,dble(m),irtc)
      if(irtc .ne. 0)then
        write(*,*)'tsih-1 ',i,ip,ic
      endif
      return
      end
      ...
      call tfreecheck('tfree-8',1,0,dble(ia),irtc)
      if(irtc .ne. 0)then
        write(*,*)'tfree ',ia,ix,ilist(1,ia-1),m
      endif
      do i=1,nfree
