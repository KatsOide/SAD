...
c 32: Er -> Ey
 ...
 9030 irtc=itfmessage(9,'General::narg',
     ...
     $     psi,xc,yc,cs,nphi1,nm)
      ...
      real*8 function atanz(x,y)
      real*8 x,y
      if(y .eq. 0.d0)then
        atanz=0.d0
      else
        atanz=(atan2(y,x)+atan2(-y,-x))*0.5d0
      endif
      return
      end
     ...
     $     w1,w2,wxx,wxy,wyx,wyy,d,t1,t2,txp,typ,atanz
      ...
      do i=1,nphi1
        ...
        do k=1,nm
          ...
          t1=2.d0*atanz(xf-xp,yf-yp)
          t2=2.d0*atanz(xf-xp,yf+yp)
        ...
        do j=1,nphi
          ...
          if(j .ne. 1 .and. j .ne. nphi)then
            ...
            a(i,      nphi2+j)=((w1+w2)*txp+(t1+t2)*typ)*.5d0
          endif
        enddo
      enddo
      call tsolvm(a,b,er,nphi1,nv,2*nm,
     $     nphi1,nphi1,nv,eps,.true.)
      do k=1,nm
        xf=xc(k)
        yf=yc(k)
        do i=1,nm
     ...
     $             (w1-w2)*er(nphi3+j,2,i)+(t1+t2)*er(nphi2+j,2,i)
            ...
            do i=1,nm
              exa1(i)=exa1(i)+(w1+w2)*er(nphi2+j,1,i)*.5d0
              eya1(i)=eya1(i)+(t1+t2)*er(nphi2+j,1,i)*.5d0
              exa2(i)=exa2(i)+(w1+w2)*er(nphi2+j,2,i)*.5d0
              eya2(i)=eya2(i)+(t1+t2)*er(nphi2+j,2,i)*.5d0
            enddo
          endif
        enddo
        do i=1,nm
