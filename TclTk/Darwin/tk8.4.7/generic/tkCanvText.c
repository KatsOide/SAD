    ...
    TextItem *textPtr;		/* Item whose bbox is to be recomputed. */
{
    ...
    */
	...
	int currianch,newanchor,anchx1,anchy1,anchx2,anchy2;
				 ...
				 * redisplayed (not used). */
{
    ...
    int xoff, yoff, xos, yos, xpos, ypos, w, h, wt, ht;
    int xsi, ysi, wsi, hsi;
    Pixmap pixmap;
    ...
    if (textPtr->rotation != 0) {
	int i,j,wd,hd,bd,dp,dxp,dyp,dwp,dhp;
	short tmpx, tmpy;
	XImage *ims, *imd;
	char *imddat,*imsdat;
	Window root;
...
/*	printf("xsi etc.:%d %d %d %d %d %d\n",xsi,ysi,wsi,hsi,dxp,dyp);*/
	...
	if (textPtr->usebg) {
	    ...
	    XPutImage(display, pixmap, textPtr->copyGC, imd, 0, 0, 0, 0, 
		wt, ht);
    ...
    */
	...
	XPutImage(display, drawable, textPtr->copyGC, imd, 0, 0, xsi, ysi, 
		w, h);
