     ...
     $           utwiss(1,nfam1,iutp),np)
        ...
        if(iwp .le. nwakep)then
          iutp=itwissp(i2)
          idx=kytbl(kwDX,idtype(ilist(2,latt(i2))))
          if(idx .ne. 0)then
            dx=rlist(latt(i2)+idx)
          else
            dx=0.d0
          endif
          idy=kytbl(kwDY,idtype(ilist(2,latt(i2))))
          if(idy .ne. 0)then
...
c      parameter (fact=1.d0/sqrt(pi2))
     ...
     $         +wbufyl(i)*(ut0(mfitdy,i)-dy))
      ...
      use iso_c_binding
      implicit none
      integer*8 kx,ktaloc,kal,kalj,ktfmalocp,kat,katj,isp0,isp1,isp2,j,i
      integer*4 irtc,lfno,n,m,lenw,l,itfdownlevel,lbegin,lend,k
      real*8 frbegin,frend
