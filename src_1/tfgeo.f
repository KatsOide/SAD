     ...
     $     posi
      ...
      if(.not. calgeo)then
        do 1030 i=1,nlat-1
          if(idtype(ilist(2,latt(i))) .eq. 20)then
            calgeo=.true.
            goto 1040
          endif
1030    continue
        ...
        if(it .eq. icMARK)then
          ...
          if(offset .ne. 0.d0)then
            ...
            if(fr .eq. 0.d0)then
              ...
              nv=kytbl(kwmax,lt)-1
              if(ideal)then
                call tmov(rlist(idval(ilist(2,latt(lxp)))+1),vsave,nv)
              else
                call tmov(rlist(latt(lxp)+1),vsave,nv)
              endif
              call qfraccomp(latt(lxp),0.d0,fr,ideal,chg)
              if(chg)then
              ...
              if(ideal)then
                call tmov(vsave,rlist(idval(ilist(2,latt(lxp)))+1),nv)
              else
          ...
          do while(.true.)
            ...
            elseif(j .eq. nlat)then
              j=0
              posi=pos(1)
            elseif(idtype(ilist(2,latt(j))) .eq. it)then
              inext(i)=j
              iprev(j)=i
              exit
            endif
...
c      h1=sqrt(1.d0+p1**2)
     ...
     $           -rlist(id+kytbl(kwE2,k))))/sin(.5d0*phi)
        ...
        elseif(kytbl(kwL,k) .ne. 0)then
          if(ideal)then
            ali=rlist(idval(ilist(2,latt(i)))+kytbl(kwL,k))
          else
            ali=rlist(latt(i)+kytbl(kwL,k))
          endif
        else
          ali=0.d0
        endif
        ...
        if(calgeo)then
          if(k .eq. icMARK)then
            if(ke .ne. 0 .and. ke1 .ne. 0)then
              if(rlist(idval(ilist(2,latt(i)))+kytbl(kwGEO,icMARK))
     $             .ne. 0)then
...
c                a=sqrt((1.d0-b)*(1.d0+b))
     ...
     $               rlist(latt(ke)+kytbl(kwCHI3,icSOL))+dchi3
          ...
          if(kytbl(kwANGL,k) .ne. 0)then
            v=rlist(latt(i)+kytbl(kwANGL,k))
            ald=rlist(latt(i)+kytbl(kwL,k))
            if(v .eq. 0.d0)then
              ...
              theta=rlist(latt(i)+kytbl(kwROT,k))
