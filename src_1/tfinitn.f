...
c      write(*,*)'tfinitstk: ',isporg
     ...
     $     isp1,isp0,isp2,ispopt,i
      integer*4 irtc,itfmessage,nc
...
c        write(*,*)'tftocontext ',str%str(1:nc)
 ...
 9000 irtc=itfmessage(9,'General::wrongtype',
     ...
     $       '"Context (Symbol ending `)"')
      ...
      type (sad_list), pointer :: kl
      type (sad_symdef), pointer :: symd
      integer*8 ka1,ki,ktaloc,isp1,isp0
      integer*4 irtc,itfmessage,m,i
      logical*4 tfcontextqk
      if(isp1+1 .ne. isp)then
        irtc=itfmessage(9,'General::narg','"1"')
        return
...
c      call tmov(ktastk(isp0+1),klist(ka1),m)
 ...
 9000 irtc=itfmessage(9,'General::wrongtype','"List of Contexts"')
      ...
      type (sad_descriptor) kx
      integer*8 isp1
      integer*4 irtc,itfmessage,istatus
      if(isp .eq. isp1)then
...
c     Initialize map/ieval array
     ...
     $     +iattrorderless
      ...
      i=itfunaloc('ReplacePart',22,3,map,ieval,0)
      map(1)=1
      ...
      i=itfunaloc('SwitchCases',37,2,map,ieval,0)
      ...
      ieval(3)=0
      i=itfunaloc('Select',41,3,map,ieval,0)
      ieval(2)=1
      ieval(1)=1
      ...
      i=itfunaloc('Append$',44,2,map,ieval,1)
      i=itfunaloc('Prepend$',45,2,map,ieval,1)
      ieval(1)=1
      ieval(2)=1
      ...
      i=itfunaloc('Position',59,4,map,ieval,0)
      ieval(1)=1
      ieval(2)=1
      ieval(3)=1
...
c      i=itfunaloc('GaussRandom',90,1,map,ieval,0)
      ...
      ieval(3)=0
      ieval(4)=0
      i=itfunaloc('Scan',96,3,map,ieval,0)
...
c      i=itfunaloc('GetGID',130,1,map,ieval,0)
      ...
      ieval(4)=0
      ieval(5)=0
      i=itfunaloc('Cases',133,2,map,ieval,0)      
      i=itfunaloc('DeleteCases',134,2,map,ieval,0)
      ieval(1)=1
      ieval(2)=1
...
c      i=itfunaloc('BidirectionalPipe',136,1,map,ieval,0)
      ...
      ieval(1)=0
      ieval(2)=0
      i=itfunaloc('SelectCases',142,2,map,ieval,0)
      ...
      ieval(1)=0
      i=itfunaloc('Extract',154,2,map,ieval,1)
...
c      i=itfunaloc('DateString$',176,1,map,ieval,0)
      i=itfunaloc('Insert',177,3,map,ieval,1)
      i=itfunaloc('Delete',178,2,map,ieval,1)
