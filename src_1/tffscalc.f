...
c     end   initialize for preventing compiler warning
     ...
     $       =utwiss(1:ntfun,0,itwissp(ibegin))
        ...
        call twmov(latt(1),twiss,nlat,ndim,.true.)
     ...
     $         aint(twiss(nlat,0,mfitny)/pi2)
          if(hstab(0) .and. vstab(0) .or. chgini)then
            call twmov(latt(1),twiss,nlat,ndim,.false.)
          endif
        endif
        if(nfam .ne. 0)then
          if(parallel)then
...
c            endif
     ...
     1         '?Too many fit conditions.',' ')
      ...
      if(wcal)then
        ...
        zcal=.false.
        do i=1,nvar
          kt=idtype(ilist(2,latt(klp(ivarele(i)))))
...
c            write(*,*)'twfit ',nlist(k),vx,wfit(i)
      ...
      xnlat=nlat
      if(idtype(ilist(2,latt(1))) .eq. icMARK)then
        offset=max(1.d0,min(xnlat,
     $       1.d0+tffsmarkoffset(latt(1))))
        lbegin=int(offset)
      ...
      if(idtype(ilist(2,latt(nlat-1))) .eq. icMARK)then
        offset=tffsmarkoffset(latt(nlat-1))
     ...
     $     lbegin,frbegin,lend,frend)
      ...
      real*8 frbegin,frend,xnlat,offset,tffsmarkoffset
      xnlat=nlat
      if(idtype(ilist(2,latt(la))) .eq. icMARK)then
        offset=max(1.d0,min(xnlat,
     $       la+tffsmarkoffset(latt(la))))
        lbegin=int(offset)
        frbegin=offset-lbegin
      else
        lbegin=la
      ...
      if(idtype(ilist(2,latt(lb1))) .eq. icMARK)then
        offset=tffsmarkoffset(latt(lb1))
        if(offset .ne. 0.d0)then
      ...
      include 'inc/MACKW.inc'
      integer*8 latt
      tffsmarkoffset=rlist(latt+kytbl(kwOFFSET,icMARK))
      if(tffsmarkoffset .ne. 0.d0)then
        tffsmarkoffset=(tffsmarkoffset-.5d0)
     $       *rlist(latt+ilist(1,latt))+.5d0
      endif
      return
      end
      ...
      nm=0
      xp=l
      if(l .ne. nlat .and. idtype(ilist(2,latt(l))) .eq. icMARK)then
        xe=nlat
        lm=l
 8111   offset=tffsmarkoffset(latt(lm))
        if(offset .ne. 0.d0)then
          xp=offset+lm
          if(xp .ge. 1.d0 .and. xp .le. xe)then
            lx=int(xp)
            if(idtype(ilist(2,latt(lx))) .eq. icMARK)then
              nm=nm+1
              if(nm .lt. nmmax)then
                lm=lx
                go to 8111
                ...
                call termes(icslfno(),
     $               '?Recursive OFFSET in',pname(ilist(2,latt(l))))
              endif
            endif
          endif
        endif
