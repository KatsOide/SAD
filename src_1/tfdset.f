      ...
      integer*8 ktdaloc,kap,kadi,isp0,mstk0,
     $     ktdhtaloc,kad,kad1,kad0,kad10,kan,kih
      integer*4 m,ihash,itfhasharg,
     $     iop0,itflistmat,irtc,minhash,maxhash
...
c        call tfdebugprint(kr,':= ',3)
 ...
 8000 klist(kad1)=klist(kad)
      ...
      integer*8 isp0,ispr,i
      integer*4 irtc,ipsbase,nrule
      logical*4 rep,same
      isp0=isp
     ...
     $     k,karg,kr,kargr,tbl)
      ...
      type (sad_deftbl), pointer :: dtbl
      integer*8 kb0,kn0,kan,kan0,ktalocr,kb,kn,isp0
      integer*4 npat,i
...
c        write(*,*)'loc.cont ',klist(klist(ktfaddr(klist(kan+7+i))+7)-3)
     ...
     $     isp1,isp0,mstk0,im
      integer*4 iup,irtc,is,itfhasharg,m,i,itfseqmatstk,ns,nh,iord0
     ...
     $       'itfhasharg-implementation error: ',3)
      ...
      elseif(ktfrefqd(k,ka))then
        if(ka .eq. 0)then
          ...
          ih=itfhash1(dtastk(ka))+int(ktastk2(ka)-ka)
        endif
      elseif(ktfsymbolqd(k,sym))then
        ih=int(sym%loc)
      elseif(ktfstringqd(k))then
      ...
      type (sad_deftbl), pointer :: dtbl
      integer*8 kad0,kad,kap,kpp,isp1,mstk0,isp0,im
      integer*4 irtc,mat,itfpmat,itfseqmatstk,
 ...
 30       call tfunsetpattbl(dtbl)
      ...
      integer*8 i,kts,kah,ks1,ks,isp1,isp0
      integer*4 irtc,m,itfmessageexp
...
c      call tfdebugprint(kx,'tfreparg-out',1)
     ...
     $     result(lx)
      ...
      integer*8 kah,isp1,isp0
      integer*4 irtc,i,m,itfmessageexp
...
c              call tfdebugprint(list%body(i),'==>',3)
     ...
     $               ktfrealq(ktastk(isp)))then
          ...
          dtastk(isp1+1)=kxcrelistm(int(isp-isp1-1),
...
c        call tfdebugprint(kx,'setarg-const',3)
     ...
     $         kx,rep,member,irtc)
      ...
      type (sad_list), pointer :: list,klx
      type (sad_pat), pointer :: pat
      integer*8 ka1,isp1,ispr
      integer*4 irtc,i,m,nrule2,itfmessageexp,j
