 ...
 9010 irtc=itfmessage(9,'General::wrongtype',
     ...
     $     italoc,itfm2l,iai
      ...
      if(tfnumlistq(itastk(1,isp1+2),iai))then
        if(ilist(2,iai-1) .ne. 2)then
          go to 9010
        endif
        dx=rlist(ilist(2,iai+1)+1)
        dy=rlist(ilist(2,iai+1)+2)
      elseif(itastk(1,isp1+2) .ne. ntfreal)then
        go to 9010
      else
        dx=vstk(ivstkoffset+isp1+2)
        dy=dx
      endif
      ...
      call csrind(ilist(1,iaind),rlist(ipipe),m1,n1,mm)
      iacm=italoc(mm*mm)
      call tclr(rlist(iacm),mm*mm)
      call csrmat(rlist(iacm),ilist(1,iaind),
     $     dx,dy,omega,rho,mode,
     $     mm,m1,n1,ic)
 ...
 9000 irtc=itfmessage(9,'General::narg','"5"')
      return
 9010 irtc=itfmessage(9,'General::wrongtype',
     $'"Pipe, d or {dx, dy}, omega/c, rho, center, mode"')
      return
      end

      subroutine csrmat(acm,ind,
     $     dx,dy,omega,rho,mode,mm,m,n,ic)
      implicit none
     ...
     $     cfy0,cfy1,cfy2,cfxa1,cfxa2,cfya,cfy1r,cfy2r,cgy,
     $     a0
      parameter (a0=0.46d0)
      if(mode .eq. 119)then
c Es resistive
...
c 33: Ey
        ...
        do i=1,m
          ...
          cfy=rdy**2*ac
          if(mode .eq. 22)then
            cfy0 =-cfx-4.d0*cfy+3.d0*rdx*rdy*ac*a0*dy/dx
            cfy1 = cfy
            cfya =-cfy
            cfxa1= cfx-cg
            cfxa2= cfx+cg
          else
            cfy0 =-4.d0*cfx-cfy++3.d0*rdx*rdy*ac*a0*dx/dy
            cfy1 =-cfy
            cfya = cfy
            cfxa1=-cfx+cg
            cfxa2=-cfx-cg
          ...
          do j=1,n
            ...
            if(j .eq. 1)then
              ...
              if(j1 .ne. 0)then
                ...
                if(im1 .eq. 0 .or. i1 .eq. 0)then
                  acm(jm,jm)=ce+cfy0
                else
                  acm(jm,jm)=acm(jm,jm)+cfya
                endif
              endif
            endif
            if(im1 .ne. 0)then
              acm(jm,im1)=cfx-cg
            elseif(j1 .ne. 0)then
              acm(jm,jm)=acm(jm,jm)+cfxa1
            endif
            if(i1 .ne. 0)then
              acm(jm,i1)=cfx+cg
            elseif(j1 .ne. 0)then
              acm(jm,jm)=acm(jm,jm)+cfxa2
            endif
          enddo
        enddo
...
c 32: Er -> Ey
        ...
        do i=1,m
          ...
          cfx=rdx**2*ac
          cfy=rdy**2*ac
          cfy0=-3.d0*rdx*rdy*ac*(1.d0+a0)
          if(mode .eq. 23)then
            cfy1 = cfx/3.d0*dy/dx
            cfy2 =-cfy0/9.d0
            cfy1r= cfy2
            cfy2r= cfy/3.d0*dx/dy
          else
            cfy1 =-cfy0/9.d0
            cfy2 = cfy/3.d0*dx/dy
            cfy1r= cfx/3.d0*dy/dx
            cfy2r= cfy1
          endif
          do j=1,n
            jm=ind(i,j)
            ...
            if(j1 .eq. 0)then
              ...
              if(im1 .eq. 0 .or. i1 .eq. 0)then
                acm(jm,jm)=cfy0*0.d0
                acm(jm,jm1)=acm(jm,jm1)+cfy2-cgy
                acm(jm1,jm)=acm(jm1,jm)+cfy2r
                if(i1 .ne. 0)then
                  acm(jm,i1)=cfy1
                  acm(i1,jm)=cfy1r
                endif
                if(im1 .ne. 0)then
                  acm(jm,im1)=acm(jm,im1)+cfy1
                  acm(im1,jm)=acm(im1,jm)+cfy1r
                endif
              else
                acm(jm,jm1)=-cgy
              endif
