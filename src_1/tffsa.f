     ...
     $     nster,nrpt1,icsmrk,itfpeeko,itfgetrecl,nl
...
c        write(*,*)'tffsa-00 ',ilist(2,klist(ilattp+1))
        call tffsalloc(klist(ilattp+1),ilist(1,ilattp))
c        write(*,*)'tffsa-01 ',ilist(2,klist(ilattp+1))
        if(.not. chguse)then
...
c          call cssetlinep(icslrecl())
     ...
     $           'Missing beamline in USE.',' ')
          ...
          lattuse=ilattp
          call tclrpara(klist(ilattp+1),ilist(1,ilattp))
 ...
 7310   call termes(lfno,
     $       'Usage: SYNCHROB_ETA nus_start nus_stop nus_step.',' ')
      ...
      elseif(abbrev(word,'ALI_GN','_'))then
        call talign(latt,word,wordp,
     $       pos,mult,lfno,exist)
        go to 30
      endif
 7000 call tfgetv(word,flv%ntouch,lfno,exist)
      if(.not. exist)then
        if(itt .ge. 0)then
          call cssetp(nextt)
...
c        write(*,*)'tffsa ',flv%nfc,nfc0
     ...
     $         flv%kfit(k1) .le. mfitchi3)then
      ...
      if(.not. geomet)then
        do i=1,nele
          ii=(i-1)/2
          if(idtype(ilist(2,latt(ilist(i-ii*2,ifklp+ii))))
     $         .eq. 20)then
            geomet=.true.
            exit
          endif
...
c      write(*,*)'tffsa-2 ',flv%iut,%LOC(rlist(flv%iut)),busy
 ...
 9000 call tfconnectk(kffs,irtcffs)
     ...
     $             max(flv%ifitp(j),flv%ifitp1(j))
      ...
      enddo
      itwissp(1)=1
      i2=max(1,min(nlat,1+int(tffsmarkoffset(latt(1)))))
      itwissp(i2)=1
