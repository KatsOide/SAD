...
c itestr
      ...
      do 20 i=npnti,npntf-1
        if(idtype(ilist(2,latt(i))).eq.4) then
          do 21 j=i-1,-nlat,-1
            k=j+nlat*((1-sign(1,j-1))/2)
            if(idtype(ilist(2,latt(k))).ne.41.and.k.ne.nlat)then
              if(k.le.npntf-1.and.idtype(ilist(2,latt(k))).eq.4) then
              else
                nquad=nquad+1
              endif
              goto 20
...
c     ::::::::::
        ...
        do 41 i=1,npair
        rlist(itr-1+i)=
     1     -rlist(idval(ilist(2,latt(ipair(1,i))))+2)*
...
cdebug------>
     ...
     z                            - xamp/trans(1,1)
        ...
        do 110 jp=1,npair
          id=idval(ilist(2,latt(ipair(1,jp))))+5
          dx=rlist(id)
          rlist(id)=halfpi
...
c    1                rlist(ia+nmo*(jp-1)),nmo,
  ...
  110   continue
        j=ia+nmo*(npair-1)
        do 112 l=1,nquad
          id=idval(ilist(2,latt(iquad(l))))+5
          j=j+nmo
          dx=rlist(id)
          rlist(id)=halfpi
          kk=istr(1,1)
...
c             akick=rlist(ix-1+jp)/rlist(idval(ilist(2,latt(ipair(1,jp))))+2)
     ...
     z                           /trans(3,4)
     z                    /sin(rlist(idval(ilist(2,latt(jst(js))))+5))
...
c       vertical scale in the graphics.
 ...
 10       nsex=0
          do i=1,nlat-1
            if(idtype(ilist(2,latt(i))).eq.icsext
     $           .and. master(i).gt.0) then
              if(tmatch(pname(ilist(2,latt(i))),patt)) then
...
c     ++ assume sextupole is a single piece
          xsb(1,i)=xs0(1,i)+
     $         0.5d0*rlist(idval(ilist(2,latt(isex(i))-1))+1)*xs0(2,i)
          xsb(2,i)=xs0(3,i)+
     $         0.5d0*rlist(idval(ilist(2,latt(isex(i))-1))+1)*xs0(4,i)
        enddo
      endif
...
c         ++ assume sextupole is a single piece
          xs0(1,i)=xs0(1,i)+
     $         0.5d0*rlist(idval(ilist(2,latt(isex(i))))+1)*xs0(2,i)
          xs0(3,i)=xs0(3,i)+
     $         0.5d0*rlist(idval(ilist(2,latt(isex(i))))+1)*xs0(4,i)
        enddo
...
c         ++ assume sextupole is a single piece
          xs(1,i)=xs(1,i)+
     $         0.5d0*rlist(idval(ilist(2,latt(isex(i))))+1)*xs(2,i)
          xs(3,i)=xs(3,i)+
     $         0.5d0*rlist(idval(ilist(2,latt(isex(i))))+1)*xs(4,i)
 16     continue
...
c       assume thin quads are attached to both ends of sexts.
        ...
        do i=1,nsex
          ls=isex(i)
          alh=0.5d0*rlist(idval(ilist(2,latt(ls)))+1)
          akh=0.5d0*rlist(idval(ilist(2,latt(ls)))+2)
...
c         ++ assume sextupole is a single piece
          ls=isex(i)
          r=rlist(idval(ilist(2,latt(ls)))+5)
          rlist(idval(ilist(2,latt(ls)))+5)=
     $         -halfpi+rlist(idval(ilist(2,latt(ls)))+4)
          k=istr(istr(1,2),1)
          istr(istr(1,2),1)=ls
          call mcrmat(latt,twiss,gammab,-ndim,0d0,0d0,
     $         a(la,i),ia,.false.,.false.,istr,1,imon,no,'Y')
          istr(istr(1,2),1)=k
          rlist(idval(ilist(2,latt(ls)))+5)=r
          call tmov(a(la,i),a(la,i+nsex),no)
          ak=rlist(idval(ilist(2,latt(ls)))+2)*
...
c     end   initialize for preventing compiler warning
      do 900 loop=1,iter
        ...
        do 100 isx=1,nsex
          ls=isex(isx)
          al=rlist(idval(ilist(2,latt(ls)))+1)
          ak=rlist(idval(ilist(2,latt(ls)))+2)
          do 10 i=1,no-1
 ...
 10   continue
      ...
      do 20 i=is1,is2
        ...
        if(mhogan(l1,l2,j)) then
          if(idtype(ilist(2,latt(j))).eq.icbend) then
            if(first) then
              iss=i
              first=.false.
            endif
            ...
            cost=cos(-rlist(idval(ilist(2,latt(j)))+5))
            sint=sin(-rlist(idval(ilist(2,latt(j)))+5))
            ddk=rlist(latt(j)+11)
            call padd(tr1,tr,20)
...
c
      if(rst) then
        do i=1,nlat-1
          ld=idtype(ilist(2,latt(i)))
          lp=latt(i)
          if(ld.eq.icbend) then
            errs(1,i)=rlist(lp+11)
            errs(2,i)=rlist(lp+5)
            rlist(lp+11)=0d0
            rlist(lp+5)=rlist(idval(ilist(2,latt(i)))+5)
          elseif(ld.eq.icquad.or.ld.eq.icsext) then
            ...
            rlist(lp+5)=0d0
            rlist(lp+6)=0d0
            rlist(lp+4)=rlist(idval(ilist(2,latt(i)))+4)
            rlist(lp+2)=rlist(idval(ilist(2,latt(i)))+2)
          endif
        enddo
      else
        do i=1,nlat-1
...
c        do  i=1,nlat-1
c          if(idtype(ilist(2,latt(i))).eq.icbend) then
...
c     character*20  name
      ...
      print *,'nbin=',nbin,' dds=',sngl(dds)
        ...
        do 2010 i=1,nlat-1
          if( .not.tmatch(pname(ilist(2,latt(i))),word) )goto 2010
          exist=.true.
          j=ilist(2,latt(i))
          k=latt(i)
          id=idtype(j)
...
c       endif
 ...
 10     j=0
        nepas=.false.
        do 12 i=1,nlat-1
          if(tmatch(pname(ilist(2,latt(i))),word)) then
            do 11 l=1,nobs
