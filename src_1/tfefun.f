      ...
      integer*8 ka1,ka,kax,km,isp1,isp2,isp0,i
      integer*4 irtc,id,narg,nc,icslfno,itfdepth,
     $     itfgetrecl,ltr0,iaf,itfopenwrite,itfmessage,
     $     itfopenappend,itfopcode,nsize
      real*8 vx,v1,v
      character*3 opcx
      character*8 char8
...
c            msgn TagS (*   *)   Hold z
 ...
 7000 isp=isp1+narg
      ...
      elseif(isp1+1 .eq. isp2)then
        ...
        kx=kxcrelistm(int(isp2-isp1),ktastk(isp1+1:isp2),
     $       k_descr(ktfoper+mtfnull))
      ...
      type (sad_symdef), pointer :: symd
      integer*8 ka1,isp1
      integer*4 irtc,narg,id,itfmessageexp
 ...
 1    k1=dtastk(isp1)
     ...
     $           ktfnonreallistqo(kl))then
        ...
        elseif(kl%head .eq. ktfoper+mtflist)then
          go to 6900
 ...
 7000 isp=isp1+narg
      ...
      integer*8 isp1,i,j,ipf0,nap0
      integer*4 irtc,itfmessage,narg,m
      logical*4 rep
      if(kf%nl .eq. 1)then
        do i=isp1+1,isp
     ...
     $               '"List of symbols"')
      ...
      elseif(narg .eq. 1)then
        if(ktfsymbolqd(dtastk(isp)) .or.
     $       ktfpatqd(dtastk(isp)))then
          kx=kxmakelist(isp1,klx)
          klx%head=ktfoper+iopc
        else
          kx=dtastk(isp1+1)
        endif          
        irtc=0
        ...
        do i=isp1+2,isp
          ...
          if(ktfrealqd(ki,vi) .and. ktfrealqd(kx,vx))then
            ...
            k1=kx
            if(tfnumberqd(k1) .and. tfnumberqd(ki))then
     ...
     $             .false.,euv,irtc)
      ...
      implicit none
      type (sad_descriptor) kx
      integer*8 isp1,k,isp0
      integer*4 irtc,iopc,itfmessage
     ...
     $     ktfstringq(ktastk(isp)) .and. ktfstringq(ktastk(isp1+1)))then
        if(tfsamestringqk(ktastk(isp),ktastk(isp1+1)))then
          kx%k=ktftrue
        else
          kx%k=0
        endif
