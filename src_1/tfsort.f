      ...
      integer*8 isp1,isp0
      integer*4 mode,irtc,narg,m,itfmessage
      narg=int(isp-isp1)
      if(narg .gt. 2)then
      ...
      type (sad_list) kl
      integer*8 isort,isp0
      integer*4 n,mode,irtc,itab(n),i,j
      logical*4 av,ins
...
c            2 Merge union with dropped index list (for Override[])
      ...
      integer*4 itab(n)
      integer*4 l,m,im,is,i1,i2,l1,l2,j1,j2,p0,p1,p2
      integer*4 itforderl
      logical*4 av
...
c     Special version of tfsortml for Real List
      ...
      integer*4 n,mode
      integer*4 itab(n)
      integer*4 m,im,is,i1,i2,l1,l2,j1,j2,p0,p1,p2
      real*8 v1,v2
...
c        enddo
      ...
      integer*8 ka1,kax,isp1,isp0,isp2,i,il,ih,kk
      integer*4 mode,irtc,j,itfcanonicalorder,m,narg,itfmessage
      real*8 vj
 ...
 410        isp=isp+1
      ...
      type (sad_list) kl
      integer*8 isp1
      integer*4 irtc,itfmessage,itfcanonicalorder,i1,i2
      real*8 v1,v2
      logical*4 av
      irtc=0
      if(av)then
...
c        write(*,*)'itforderl ',i1,i2,itforderl
     ...
     $         '"Real number","as the result of order-function"')
        ...
        isp=isp+1
        dtastk(isp)=k1
        call tfefunref(isp1,kx1,.true.,irtc)
        isp=isp1-1
        if(irtc .ne. 0)then
          itforderl=-1
          return
     ...
     $         '"Real number","as the result of order-function"')
      ...
      integer*4 irtc,itfcanonicalorder,itfmessage
