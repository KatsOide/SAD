...
c      write(*,*)'tfreeshared ',kpb,klist(kpb-1),irtc
      ...
      integer*8 kax,isp0,i
      integer*4 m,irtc,itfmessage
...
c        call tfdebugprint(k,'recallshared',3)
      ...
      use tfstk
      implicit none
      integer*8 k,ka,kap,kh,ki,kt,isp0,j
      integer*4 i,m
      ka=ktfaddr(k)
      kt=k-ka
        ...
        if(ktfobjq(kh) .and. ktfnonsymbolq(kh))then
          do j=isp0,isp
            if(ktastk(j) .eq. kh)then
              klist(kap+1)=iand(ktfmask,kh)+ktastk2(j)
              exit
            endif
          enddo
        endif
...
c          enddo
      ...
      use tfstk
      implicit none
      integer*8 k,ka,kt,ki,kh,isp0,i
      integer*4 irtc,itfmessage,ni,n
      irtc=0
