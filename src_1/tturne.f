...
c      vcsin=0.d0
      ...
      if(frbegin .ne. 0.d0)then
        call qfracsave(latt(lbegin),vsave,nvar,.true.)
        call qfraccomp(latt(lbegin),frbegin,1.d0,ideal,chg)
        call tturne1(trans,cod,beam,
     $       iatr,iacod,iabmi,plot,sol,rt,lbegin,lbegin)
        if(chg)then
          call qfracsave(latt(lbegin),vsave,nvar,.false.)
        endif
        ls=lbegin+1
      else
        ls=lbegin
     ...
     $     iatr,iacod,iabmi,plot,sol,rt,ls,lend-1)
      if(frend .ne. 0.d0)then
        call qfracsave(latt(lend),vsave,nvar,.true.)
        call qfraccomp(latt(lend),0.d0,frend,ideal,chg)
        call tturne1(trans,cod,beam,
     $       iatr,iacod,iabmi,plot,sol,rt,lend,lend)
        if(chg)then
          call qfracsave(latt(lend),vsave,nvar,.false.)
        endif
      endif
      if(plot)then
...
c     below is incorrect for fra <> 0
              call qfracsave(latt(lx),vsave,nvar,.true.)
              call qfraccomp(latt(lx),fra,frb,ideal,chg)
              if(.not. chg)then
                fr=0.d0
                go to 8101
              endif
     ...
     $             lx,lx)
              codplt=cp0
              calint=int0
              call qfracsave(latt(lx),vsave,nvar,.false.)
...
c          trf0=phis*c*p0/h0/omega0/hvc0*vceff
     ...
     $             dtfcopy1(kxm2l(cod,0,6,1,.false.))
        ...
        dir=rlist(latt(l)+ilist(1,latt(l)))
        kl=kytbl(kwL,lele)
        if(kl .ne. 0)then
          al=rlist(lp+kl)
        else
