      subroutine tcorr(word,latt,pos,mult,master,lfno)
      use tfstk
      use ffs
      use tffitcode
      implicit real*8 (a-h,o-z)
      integer*8 latt(nlat),le,le1
      dimension mult(nlat),pos(nlat)
          ...
          do 110 j=i0,ie,istep
            if(master(j) .ne. 0 .or. id .eq. 41)then
              ...
              elseif(id .eq. 20)then
                ...
                le1=le+1
                if(.not. add)then
                  dx1=rlist(idval(ilist(2,latt(j)))+3)-delx
                  dy1=rlist(idval(ilist(2,latt(j)))+4)-dely
                else
                  dx1=-delx
                  dy1=-dely
                endif
              elseif(id .eq. 31)then
                ...
                le1=le+1
                if(.not. add)then
                  dx1=rlist(idval(ilist(2,latt(j)))+13)-delx
                  dy1=rlist(idval(ilist(2,latt(j)))+14)-dely
                else
                  dx1=-delx
                  dy1=-dely
                endif
