c     Don't confuse, Emacs. This is -*- fortran -*- mode!
     ...
     $     ierrorexp,ifunbase,initmessage,levelcompile,iordless
      integer*4 
     $     levele,levelp,lgeneration,ltrace,modethrow
      end module
...
c     $      type,ival,idx
     ...
     $     kcpthre  = 2**35, nlarge = 2**30, nitaloc0 = 2**30
      integer*4 icbk
     ...
     $     ktfnan   =int8(z'fff8000000000000'))
      ...
      integer*8 mstk,isp,ivstkoffset,ipurefp,napuref,isporg
...
c     write(*,*)'with ',ilist(1,ka-1),ktfaddr(klist(ka-2))
 ...
 10     if(present(kl))then
     ...
     $           ilist(1,ktfaddr(kxnulls)-1)+1
        ...
        integer*8 isp1
        ktfcompose=ktfaddr(
     $       kxcrelistm(int(isp-isp1),ktastk(isp1+1:isp),dtastk(isp1)))
        if(present(kl))then
