...
20        posa(i)=pos(i)+pos(nlat)
     ...
     1'SET TITLE SIZE -1.6'
      ...
      do i=istart,iend
        if(idtype(ilist(2,latt(i))) .eq. icMARK)then
          posa(i)=posa(i-1)
        endif
      enddo
      fin=.false.
11    do 10 i=istart,iend
 ...
 12     if(kytbl(kwL,id) .gt. 0)then
        ...
        if( tmatch(pname(ilist(2,latt(i))),patt) )then
          exist=.true.
          ln=min(8,lene(pname(ilist(2,latt(i)))))
          name=' '
          name(9-ln:8)=pname(ilist(2,latt(i)))
c          xs=max(xc,REAL(ps))
          xs=REAL(ps)
          xs=max(xc,xs)
...
c     write(*,*)'tdlat ',pname(ilist(2,latt(i))),xc,yc,dx,dy
          write(lfnd,9001)
