...
c     end   initialize for preventing compiler warning
      ...
      zcal=.true.
      if(cell)then
         call twmov(latt(1),twisss,1,0,.true.)
      endif
      ibegin=1
      rstab=0.d0
      if(fitflg)then
     ...
     $         zcal,wcal,parallel,lout,error)
          if(error)then
            if(irtc .eq. 20001)then
              ...
              bestval(1:nvar)=valvar(1:nvar)
              if(cell)then
                call twmov(latt(1),twisss,1,0,.false.)
              endif
              exit do200
            endif
          endif
...
c                    enddo
              ...
              do kc=1,nvar
                ...
                wvar(kc)=tweigh(ilist(2,latt(klp(i))),
     $               idtype(ilist(2,latt(klp(i)))),
     $               ivvar(kc),bestval(kc),v00,absweit)
                if(.not. nderiv)then
                  nderiv=idtype(ilist(2,latt(klp(i)))) .eq. icSOL
                endif
              enddo
...
c     $                 /2.d0/dvkc/wvar(kc)
 ...
 1082       if(newton)then
     ...
     $             df,dval,wlimit,wexponent,nqcol,nvar)
            ...
            do ii=1,nvar
              ...
              call tffsvlimit(ii,i,ilist(2,latt(klp(i))),valvar(ii),
     $             bestval(ii),
 ...
 9000 call termes(lfno,'?Too many conditions*variables.',' ')
      ...
      do ii=1,nvar
        i=ivarele(ii)
        call tffsvlimit(ii,i,ilist(2,latt(klp(i))),
     $       valvar(ii),valvar(ii),
     $       vl,vlim,vl1,vl2,ivvar,ival,nvar,limited1,dlim)
        if(limited1)then
          limited=.true.
 ...
 2009 limited=.true.
      ...
      implicit none
      integer*8 kx,isp1
      integer*4 id,ld,irtc,level,itfuplevel,ltyp,itfdownlevel,k
      real*8 x
