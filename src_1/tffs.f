     ...
     $     bradprev
      integer*8 ilattp
      integer*4 nflag0,nlat,np0,nturn,isynch,lspect,nspect,
     ...
     $     valvar(:),valvar2(:,:)
      integer*4, pointer, dimension(:) :: mult,iele,iele1,iele2,
     ...
     $     geo(:,:,:),vlim(:,:),beamsize(:,:)
      ...
      contains
        ...
        call c_f_pointer(c_loc(ilist(1,ifele2)),iele2,[nlat])
     ...
     $     'PDEPY   '/)
      ...
      implicit none
      integer*8 ktaloc
      integer*8 , target :: latt0(nl)
      integer*4 nl,j,l,ntwis,k,i,itehash
      if(idtype(ilist(2,latt0(1))) .ne. icMARK)then
      ...
      LOOP_L: do l=1,nlat-1
        j=ielmhash+itehash(pname(ilist(2,klist(ilattp+l))),MAXPNAME)
        LOOP_K: do k=1,ilist(1,j)
          i=ilist(1,k+ilist(2,j)-1)
          if(i .ge. l)then
            exit LOOP_K
          endif
          if(ilist(2,klist(ilattp+i)) .eq.
     $         ilist(2,klist(ilattp+l)))then
...
c      call tfree(ifibzl)
     ...
     $     icslfn1,icsmrk,icslrecl,itfmessage
