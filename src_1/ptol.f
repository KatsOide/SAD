  ...
  20  continue
     ...
     1                          'rot_q**2 ym_s**2  yc**2/By '
      ...
      do 100 j=0,nstr
        ...
        ida=0
        do 90 i=is,if
          if(idtype(ilist(2,latt(i))).eq.icquad .or.
     1      idtype(ilist(2,latt(i))).eq.icbend .or.
     1      idtype(ilist(2,latt(i))).eq.icsext ) then
            id=idtype(ilist(2,latt(i)))
            if(id.eq.icbend.and.
     $           rlist(idval(ilist(2,latt(i)))+8).eq.0d0)goto 90
...
c     (vsb=K1 of bend)
     ...
     1              *dcmplx(cos(twiss(i1,idp,3)),sin(twiss(i1,idp,3)))
            ...
            ip=latt(i)
c           ip=idval(ilist(2,latt(i)))
            als=rlist(ip+1)
            vs=rlist(ip+2)
            vsa=rlist(idval(ilist(2,latt(i)))+2)
            vsb=rlist(ip+8)
            e2s=rlist(ip+4)
   ...
   60       continue
            do 70 n=1,ndiv
              ...
              if(id .eq. icbend)then
                rlist(idval(ilist(2,latt(i)))+2)=vsa*rn
                rlist(ip+2)=vs*rn
                rlist(ip+4)=0.d0
                rlist(ip+8)=vsb*rn
              else
   ...
   70       continue
            ...
            if(id .eq. icbend)then
              rlist(ip+2)=vs
              rlist(idval(ilist(2,latt(i)))+2)=vsa
              rlist(ip+4)=e2s
              rlist(ip+8)=vsb
            else
              rlist(ip+2)=vs
