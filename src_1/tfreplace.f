      ...
      integer*8 isp1,i,j
      integer*4 irtc,nrule,itfmessage
 ...
 9000 isp=isp1
      ...
      integer*8 kair,isp0,isp2,isp1,ispr,mstk0,i
      integer*4 irtc,m,nrule,itfmessageexp,itfpmat,iop
     ...
     $       ivstk2(2,i) .eq. 0
        if(ktfnonrealqd(kp) .and. ivstk2(2,i) .ne. 0)then
          ...
          if(m .ge. 0)then
            ...
            if(isp .gt. isp2)then
              call tfreplacesymbolstk(kx,isp2,int(isp-isp2)/2,kx,
     $             .false.,rep1,irtc)
            endif
            call tfresetpat(kp)
            isp=isp0
     ...
     $     scope,rep,irtc)
      ...
      integer*8 ka1,kas,isp1,ispr,j
      integer*4 irtc,i,m,nrule,itfmessageexp,id
...
c          endif
      ...
      use tfstk
      implicit none
      integer*8 loc,ispr,j
      integer*4 nrule,jm,jl,jh,iag
      jl=1
      jh=nrule
      do while (jh .ge. jl)
        jm=(jl+jh)/2
      ...
      use tfstk
      implicit none
      integer*4 n,i,j,n1,ig0,ig1
      integer*8 kai,kz0,ispr,isp1
...
c%%%%%%%%%%%%%%%%%%%%% need modification!!!
     ...
     $       .and. ip1 .le. ip2)
      ...
      implicit none
      type (sad_pat), pointer :: pat
      integer*8 kp,kap,kx,i,isp1,isp2,ispb,ispe
      logical*4 rep
...
c        endif
     ...
     $     scope,rep,irtc)
      ...
      integer*8 ispr,isp1,j
      integer*4 nrule,irtc,i
      ...
      integer*8 kai,ki,ki1,ka1,kai1,ki2,ksave,ispr,ispj,ispa,ispb
      integer*4 nrule,irtc,i
...
c        ilist(2,ktfaddr(k2)-3)=ior(ilist(2,ktfaddr(k2)-3),kmodsymbol)
     ...
     $       kx,.true.,rep,irtc)
      ...
      integer*8 kai,ki,ka1,ksave,ispr,isp1,ispi,j
      integer*4 nrule,irtc,i
     ...
     $     kx,.true.,rep1,irtc)
      ...
      integer*8 isp1,isp0,i,ispopt
      integer*4 nopt,j,irtc,lenw
      logical*4 rep
 ...
 1    irtc=0
      ...
      integer*8 kx,ki,k1,isp1,isp0,isp2,isp3,isp4,i
      integer*4 irtc,n,itfmessage,j
      if(isp1 .eq. isp)then
        kx=kxnulll
        irtc=0
        return
