...
static integer4 save_stk_frame(void) {
  integer8 isp0 = isp;	/* Save current SAD stk frame pointer for rewinding */
...
static void restore_stk_frame(integer8 isp0) {
...
static int Fork(integer8 *isp1, integer8 *kx,
		integer4 *irtc) {
  real8 vx;
...
static int Wait(integer8 *isp1, integer8 *kx,
		integer4 *irtc) {
  integer8 kax;
  pid_t pid;
  int status;
...
static int Wait4(integer8 *isp1, integer8 *kx,
		 integer4 *irtc) {
...
static int Kill(integer8 *isp1, integer8 *kx,
		integer4 *irtc) {
  pid_t pid;
  int sig;
...
static int Sleep(integer8 *isp1, integer8 *kx,
		 integer4 *irtc) {
  int status;
...
static int SigPending(integer8 *isp1, integer8 *kx,
		      integer4 *irtc) {
  integer8 isp0;
  sigset_t pending;
  int signo, status;
...
static int SigSuspend(integer8 *isp1, integer8 *kx,
		      integer4 *irtc) {
  integer8 ia;
  integer8 isp2;
  sigset_t mask;
...
static int SigProcMask(integer8 *isp1, integer8 *kx,
		       integer4 *irtc) {
...
static int SigAction(integer8 *isp1, integer8 *kx,
		     integer4 *irtc) {
...
static int Environments(integer8 *isp1, integer8 *kx,
			integer4 *irtc) {
  extern char **environ;
  integer8 isp0;
  char **ep;
...
static int GetEnv(integer8 *isp1, integer8 *kx, 
		  integer4 *irtc) {
  integer8 ka;
  char *buf;
...
static int SetEnv(integer8 *isp1, integer8 *kx,
		  integer4 *irtc) {
  integer8 ia1, ia2;
#if !HAVE_BSD_SETENV
  char *buf;
...
static int UnsetEnv(integer8 *isp1, integer8 *kx,
		    integer4 *irtc) {
  integer8 ia;
...
static int GetDirectory(integer8 *isp1, integer8 *kx, 
			integer4 *irtc) {
  char *buf;
...
static int SetDirectory(integer8 *isp1, integer8 *kx,
			integer4 *irtc) {
  integer8 ia;
  char *expanded, *buf;
...
static int System(integer8 *isp1, integer8 *kx,
		  integer4 *irtc) {
  integer8 ka;
  int status;
...
static int GetPGID(integer8 *isp1, integer8 *kx,
		   integer4 *irtc) {
  real8 vx;
  pid_t  pid = 0;
  pid_t pgid;
...
static int SetPGID(integer8 *isp1, integer8 *kx,
		   integer4 *irtc) {
  pid_t  pid;
  pid_t pgid;
...
static int Get##type(integer8 *isp1, integer8 *kx, \
		     integer4 *irtc) {			\
  real8 vx; \
  if(isp != *isp1 + 1 || ktastk(isp) != ktfoper + mtfnull) { \
    *irtc = itfmessage(9, "General::narg", "\"0\""); \
