...
c     *     klp(iele1(k)) == k if singlet or head of multipole elements
     ...
     $           .not. tmatch(pname(ilist(2,latt(k))),word))then
              cycle LOOP_K_2
            endif
            ivck=0
          endif
...
c          write(*,*)'tffsfreefix ',i,k,ivi,valvar2(i,1),valvar2(i,2)
 ...
 1210     continue
          ...
          elseif(comp)then
            cycle LOOP_I_3
          elseif(.not. tmatch(pname(ilist(2,latt(klp(kk)))),word))then
            cycle LOOP_I_3
          endif
          if(.not. found)then
            call cssetp(next)
          ...
          found=.true.
          itk=idtype(ilist(2,latt(klp(kk))))
          if(itk .ne. it)then
            if(word1 .ne. ' ')then
              ivk=1
 1110         continue
 ...
 1220     continue
        ...
        if(.not. found .and. .not. wild)then
          do k=1,nele
            if(tmatch(pname(ilist(2,latt(klp(k)))),word))then
              call cssetp(next)
              found=.true.
              go to 1
            endif
