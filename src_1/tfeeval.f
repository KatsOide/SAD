     ...
     $     ktfreallistqo(list),ref,irtc)
      ...
      integer*4 irtc,ns
      integer*8 isp1,isp10,isp11,isp0,j
      integer*4 level,itfgetrecl,mf,i1,level1,i,
     $     itfmessage,lpw,l,itfdownlevel
...
c          call tfdebugprint(ktastk(isp),' ',3)
 ...
 9000 level=max(0,level-1)
      ...
      integer*8 ki,iaat,isp0
      integer*4 mf,irtc,i,m
      logical*4 av
      irtc=0
      if(m .le. 0)then
        return
...
c        call tmov(klist(ka+1),ktastk(isp+1),m)
      ...
      integer*8 kaa,kopc,kax,isps,isp2,ipf0,naf0,isp1
      integer*4 irtc,nc,itfmessage,ns,ls,ind
      real*8 ffval,vx
      character*256 name
      character*12 inds
      logical*4 exist,ref
