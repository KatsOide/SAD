      ...
      integer*8 isp1,isp11,isp2,j,i
      integer*4 irtc,itfgetrecl,narg,lfn,nc,lpw,itfmessage,itfgetlfn
      logical*4 exist
      narg=int(isp-isp1)
      if(narg .le. 1)then
 ...
 10   call tfreestringbuf(strb)
      ...
      integer*8 isp1
      integer*4 irtc,itfmessage,icslfni,icslfno
      logical*4 read
      itfgetlfn=0
      if(isp .le. isp1)then
 ...
 100  irtc=0
      ...
      integer*8 isp1,isp2,j,i,isp11
      integer*4 irtc,narg,lfn,itfgetlfn,nc,itfmessage
      narg=int(isp-isp1)
      if(narg .le. 1)then
        irtc=itfmessage(9,'General::narg','"2 or more"')
        return
...
c      enddo
      ...
      type (sad_descriptor) kx
      integer*8 isp1
      integer*4 irtc,icslfno,itfgetrecl,nret,itfmessage
     ...
     $       .true.,irtc)
      ...
      type (sad_strbuf), pointer :: strb
      integer*8 isp0
      integer*4 lfno,irtc,nc,lrec,nret,irtc1
 ...
 9000 call tfreestringbuf(strb)
      ...
      integer*8 ii,ka0,kadi,kad,ka,isp1,isp00,isp0,ispv,i
      integer*4 irtc,kk,iop,icomp
      icomp=0
     ...
     $     lfn,icsstat,icslinep,linep,itf
      logical*4 rec,csrec
      isp0=isp
      isp=isp+1
      dtastk(isp)=k
...
c          enddo
      ...
      integer*8 isp1,isp0
      integer*4 irtc,n1,narg,itfmessage,lfn,i1
 ...
 9000 irtc=itfmessage(9,'General::wrongval',
     ...
     $     ' BinaryFloat, Real, String, or Word is","for 2nd arg"')
      ...
      include 'inc/TFREADOPT.inc'
      integer*8 kxi,isp0,isp2
      integer*4 lfn,irtc,kk,m
      logical*4 mult
      type(ropt) opts
      isp2=isp
      do kk=1,m
...
c        kx=kxsalocb(-1,jlist(is+isw-1,ib),nc)
 ...
 1    kx=kxsalocb(-1,buff(isw:),nc)
      ...
      implicit none
      type (sad_descriptor) kx
      integer*4 irtc,lfn,itfmessage,fgetc,mode,narg,
     $     ibuf(2),jbuf(4),i,nb,          ispopt
      real*8 rbuf,vx
      real*4 sbuf(2)
      character bbuf(8)
      ...
      integer*4 nopt
      parameter (nopt=1)
      integer*8 kaopt(nopt),isp1,isp0
...
c     $         char(str(i)),'$'
     ...
     $     .not. ktfstringqd(dtastk(isp),str))then
      ...
      type (sad_descriptor) kx
      type (sad_string), pointer :: str
      integer*8 ka,kfromr,isp1
      integer*4 irtc,ifile,itfopenread,
...
c      endif
      ...
      implicit none
      type (sad_descriptor) kx
      integer*8 kfromr,isp1
      integer*4 irtc,itfmessage,iu
      if(isp .ne. isp1+1)then
        irtc=itfmessage(9,'General::narg','"1"')
        return
     ...
     $     int8(3),int8(0),' ')
      ...
      type (sad_string), pointer :: str
      integer*8 isp1
      integer*4 irtc,lw,i,tftmpnam,getpid,lm
      character*4096 buff
      ...
      integer*8 isp1
      integer*4 irtc,lfn,ioff,ls,   ierr
 ...
 9000 irtc=itfmessage(9,'General::wrongtype',
     ...
     $       'EndOfFile|BeginningOfFile|CurrentPosition]"')
      ...
      type (sad_descriptor) kx
      integer*8 isp1
      integer*4 irtc,narg,lfn,itfmessage,itfgetlfn
      narg=int(isp-isp1)
      if(narg .ne. 1)then
        irtc=itfmessage(9,'General::narg','"1"')
        return
