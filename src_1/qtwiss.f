     ...
     $     j,mfr,itgetfpe,k,ibb,ibg,ntfun
      integer*8 le,lp,ld
...
c     end   initialize for preventing compiler warning
 ...
 1200     continue
          if(rlist(latt(l1)+ilist(1,latt(l1))) .gt. 0.d0)then
 ...
 1400     continue
          ...
          if(rlist(latt(l1)+ilist(1,latt(l1))) .lt. 0.d0)then
 ...
 2200     phi=rlist(lp+kytbl(kwANGL,icMULT))
          ...
          if(rlist(latt(l1)+ilist(1,latt(l1))) .ge. 0.d0)then
...
c      write(*,*)'qtrans ',la,lb,la1,lb1,fra,frb
     ...
     $     trans(4,5),cod(6),fr1,fr2,gb0,gb1,dgb
      ...
      call qfracsave(latt(l),vsave,nvar,.true.)
      call qfraccomp(latt(l),fr1,fr2,ideal,chg)
      gb0=gammab(l)
      gb1=gammab(l+1)
      dgb=gb1-gb0
...
c          enddo
        ...
        gammab(l+1)=gb1
        if(chg)then
          call qfracsave(latt(l),vsave,nvar,.false.)
        endif
      else
      ...
      logical*4 save
      if(ideal)then
        i=idval(ilist(2,latt1))
      else
        i=latt1
      endif
      if(save)then
        nvar=kytbl(kwMAX,idtype(ilist(2,latt1)))-1
      ...
      lt=idtype(ilist(2,latt))
      if(ideal)then
        ip=idval(ilist(2,latt))
      else
        ip=latt
      endif
